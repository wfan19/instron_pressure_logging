...
void loop() {
  ...
  //Serial.println("Logging: " + String(new_logging));

  // Read the pressure, and print it if we are logging
//  float pressure_volts = analogRead(pressure_sensor_pin);
...
//  last_logging = new_logging;
  ...
  float pressure_volts = analogRead(pressure_sensor_pin);
  Serial.println("Analog read: " + String(pressure_volts));
}
...
float v_to_pressure(float voltage) {
  ...
  float pressure = (P_MAX - P_MIN)/(0.8 * V_SUPPLY) * (voltage - 0.1 * V_SUPPLY) + P_MIN;
  return pressure;
}
